...
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Append only if report is valid
    if not is_target_admin or (is_target_admin and allow_admin_reports):
        reports.append(report)

    # Routing
    if is_target_admin:
        if allow_admin_reports:
            if owner_id:
                # TODO: implement send to owner
                await update.message.reply_text("✅ Report submitted to the owner.")
            else:
                await update.message.reply_text("❌ Owner not configured; cannot report admin.")
        else:
            await update.message.reply_text("❌ Admins cannot be reported in this group.")
    else:
        # TODO: implement send to on-duty admins
...
async def check_cross_group_restriction(user_id: int) -> bool:
    ...
    """
    return len(cross_group_bans.get(user_id, set())) >= CROSS_GROUP_BAN_THRESHOLD# ADMIN SAFETY
# -----------------------------
async def can_moderate(admin_user, target_user, is_owner=False):
    if target_user.is_bot:
        return False, "Bots cannot be moderated."
