...
user_sessions = {}
admin_sessions = {}
captcha_states = {}  # {user_id: {'tries': int, 'lock_time': datetime|None, 'group_id': int}}

# Sample rules (customize)
RULES_TEXT = """
Welcome to the group! Please read and agree to these rules:
1. Be respectful.
2. No spam.
3. Follow all guidelines.
Click 'Agree' to continue and complete verification.
"""
...
async def get_db_connection():
    try:
        ...
        await conn.execute("""CREATE TABLE IF NOT EXISTS user_stats (
            ...
            warns INT DEFAULT 0, violations_today INT DEFAULT 0, last_anon_link_date DATE DEFAULT NULL, verified BOOLEAN DEFAULT FALSE, PRIMARY KEY (group_id, user_id));""")
        ...
        await conn.execute("""CREATE TABLE IF NOT EXISTS global_bans (
            user_id BIGINT, group_id BIGINT, reason TEXT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
async def update_setting_db(group_id, key, val):
    ...
    conn = await get_db_connection()
    if not conn: return
    await conn.execute(f"UPDATE group_settings SET {key}=%s WHERE group_id=%s", (val, group_id))
...
async def set_user_verified(group_id, user_id, verified=True):
    conn = await get_db_connection()
    if not conn: return
    await conn.execute("UPDATE user_stats SET verified = $1 WHERE group_id=$2 AND user_id=$3", verified, group_id, user_id)
    await conn.close()

async def is_user_verified(group_id, user_id):
    conn = await get_db_connection()
    if not conn: return False
    row = await conn.fetchrow("SELECT verified FROM user_stats WHERE group_id=$1 AND user_id=$2", group_id, user_id)
    await conn.close()
...
async def cast_vote(update: Update, context: ContextTypes.DEFAULT_TYPE, pid, vote, auto=False, group_context=None):
    ...
    if prop['action_type'].startswith('blacklist_'):
        ...
        if admin_pool_count == 0 and owner_id:
            ...
            owner_weight = 50.0
            admin_weight = 50.0 / max(admin_pool_count, 1)
    ...
    for v in all_votes:
        ...
        if uid in current_admin_ids and choice == 'yes':
            if uid == owner_id and not prop['action_type'].startswith('blacklist_'):
...
async def unban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if conn:
            ...
            await conn.execute("DELETE FROM global_bans WHERE group_id=$1 AND user_id=$2", update.effective_chat.id, target.id)
            await conn.close()
...
async def warn_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    target = await get_target_user(update, context)
    if not target: return await update.message.reply_text("❌ Not found.")
    if await is_admin(update, target): return await update.message.reply_text("🛡 Immune.")
...
# --- New Welcome & Verification Handler ---
async def welcome_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    gid = update.effective_chat.id
    # Check if bot can mute (once per group, store in cache or DB if needed)
    bot_member = await update.effective_chat.get_member(context.bot.id)
    if not bot_member.can_restrict_members:
        # Notify owner once (assume we have a way to track notified, e.g., cache)
        if gid not in group_settings_cache.get('notified_no_perm', []):
            owner_id = (await update.effective_chat.get_member(BOT_CREATOR_ID)).user.id if BOT_CREATOR_ID else None
            if owner_id:
                await context.bot.send_message(owner_id, "⚠️ Bot lacks restrict permissions in group. Welcome verification disabled.")
            group_settings_cache['notified_no_perm'] = group_settings_cache.get('notified_no_perm', []) + [gid]
        return  # Don't send welcome
    for new_member in update.message.new_chat_members:
        user_id = new_member.id
        if await is_user_verified(gid, user_id):
            continue
        # Mute new user
        try:
            await update.effective_chat.restrict_member(user_id, ChatPermissions(can_send_messages=False))
        except Exception as e:
            logger.error(f"Mute new member error: {e}")
            continue
        # Send welcome with agree button
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("Agree", callback_data=f"agree_{user_id}_{gid}")]])
        welcome_msg = await update.message.reply_text(RULES_TEXT, reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN)
        # Auto-delete after 5 min if not completed (schedule job)
        context.job_queue.run_once(delete_message, 300, data={'chat_id': gid, 'message_id': welcome_msg.message_id})
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("agree_"):
        _, user_id, gid = data.split("_")
        user_id = int(user_id)
        gid = int(gid)
        if user_id != query.from_user.id:
            await query.answer("This is not for you!", show_alert=False)
            return
        # Generate captcha
        ...
        buttons = [InlineKeyboardButton(str(opt), callback_data=f"captcha_{user_id}_{gid}_{opt}_{answer}") for opt in options]
        keyboard = InlineKeyboardMarkup([buttons])
        await query.edit_message_text(f"Verify you're human: What is {num1} + {num2}?", reply_markup=keyboard)
    elif data.startswith("captcha_"):
        _, user_id, gid, selected, answer = data.split("_")
        user_id = int(user_id)
        gid = int(gid)
        ...
        if user_id != query.from_user.id:
            ...
            return
        # Check tries
        state = captcha_states.get(user_id, {'tries': 0, 'lock_time': None, 'group_id': gid})
        if state['lock_time'] and datetime.now() - state['lock_time'] < timedelta(hours=24):
            await query.edit_message_text("❌ Locked. Try again in 24 hours.")
            return
        if selected == answer:
            await set_user_verified(gid, user_id, True)
            try:
                chat = await context.bot.get_chat(gid)
                await chat.restrict_member(user_id, ChatPermissions(can_send_messages=True))
                # Delete welcome immediately
                await query.message.delete()
            except Exception as e:
                logger.error(f"Unmute error: {e}")
            await query.answer("✅ Verified! You can now send messages.", show_alert=False)
            captcha_states.pop(user_id, None)
        else:
            state['tries'] += 1
            captcha_states[user_id] = state
            if state['tries'] >= CAPTCHA_MAX_TRIES:
                state['lock_time'] = datetime.now()
                await query.edit_message_text("❌ Too many tries. Try again in 24 hours.")
                captcha_states[user_id] = state
                return
            await query.edit_message_text("❌ Wrong! Try again.")
            # Regenerate captcha
            num1 = random.randint(1, 10)
            num2 = random.randint(1, 10)
            answer = num1 + num2
...
if __name__ == "__main__":
    ...
    app.add_handler(CommandHandler("ping", ping_voters))
    app.add_handler(CommandHandler("cancel", cancel_proposal))
    ...
    app.add_handler(CommandHandler("manage", manage_group))
    app.add_handler(CommandHandler("filter", add_filter))
    app.add_handler(CommandHandler("stopfilter", del_filter))
    ...
    app.add_handler(CommandHandler("anon_link", anon_link))
    app.add_handler(CommandHandler("toplevel", toplevel))
    app.add_handler(CommandHandler("me", me_stats))
    app.add_handler(CommandHandler("reset", reset_leaderboard))
    ...
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.ChatType.GROUP, group_police))
